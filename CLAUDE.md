...
│   │   ├── chart-annotations.ts # ChartAnnotation union type (CandleMarker, HLine, SignalZone) + IndicatorParams struct
│   │   ├── candle-patterns.ts   # detectCandlePatterns() — 11 single/multi-candle formations with confidence scoring
│   │   ├── support-resistance.ts # findSupportResistance() — pivot clustering → horizontal S/R levels
│   │   ├── indicator-signals.ts  # detectIndicatorSignals() — per-candle multi-signal convergence scoring
...
│   │   └── Chart.svelte        # SVG candlestick chart with SMA/EMA/BB overlays + RSI/MACD sub-pane; ⚙ settings panel for per-session indicator periods; exports indicator period props (bind:) + accepts annotations: ChartAnnotation[] prop for overlay rendering
...
│   ├── History.svelte        # Historical chart view (preset selector, asset dropdown, Chart with warm-up candles, Analysis panel) — rendered by App
...
- CSS colours, border-radius, and font sizes must use the custom properties defined in `src/app.css` (`:root`). Categories: `--color-surface-*` (panel/element backgrounds), `--color-border[-subtle]` (borders and dividers), `--color-text-*` (text hierarchy), `--radius-*` (border-radius), `--text-*` (font sizes), `--view-max-width` / `--view-padding` (view layout). Only hardcode a value when none of the existing tokens fit; if the value will be reused across two or more components, add a new variable to `app.css` instead. SVG presentation attributes (`stroke=`, `fill=`) cannot use CSS variables and may stay as literals.
...
- Session state in `sessionStorage` must also be versioned (see `SESSION_KEYS` in `config.ts`); it clears automatically when the browser session ends so it is appropriate for ephemeral UI state like the active view
...
- **Chart indicator settings** — `Chart.svelte` exposes a ⚙ toggle in the header that reveals an inline settings panel with number inputs for SMA fast/slow, EMA fast/slow, BB period/stddev, RSI period, and MACD fast/slow/signal. Settings are per-component-instance and not persisted (reset on page reload). Alerts and proposals are unaffected. All ten period variables are `export let` so `History.svelte` can read their current values via `bind:` — this has no visible effect on the dashboard (live chart) where those props are not bound.
- **History view analysis panel** — Below the chart in `History.svelte`, three on-demand analysis buttons scan the loaded candles and produce chart annotations + a results list. **Candle Patterns** detects 11 formations (Doji, Hammer, Engulfing, Star, Soldiers/Crows) and marks each with a triangle above/below the candle. **Support & Resistance** finds horizontal price levels where the chart reversed ≥2 times and draws dashed lines across the chart. **Indicator Signals** finds candles where ≥2 independent indicator signals (RSI, MACD cross, BB breach, SMA/EMA cross) coincide and highlights those columns. All three modes produce a confidence badge (Low/Medium/High/Strong). Results are ephemeral — cleared when the user picks a different asset or preset, or clicks Clear. Analysis runs against the full loaded candle array (including warm-up candles) for accurate indicator values; only matches within the visible window are annotated.
...
- **History view warm-up candles** — Each History preset fetches more candles than it displays. The extra candles (200 for 1D/1W/1M/6M/1Y, 100 for 3D/3Y/5Y, 50 for ALL) are used only for indicator warm-up (RSI, MACD, EMA) so that every visible bar shows accurate values. The warm-up candles are never rendered. All presets fit in a single Binance REST call (under the 1000-candle limit). Preset intervals: 1D=5m, 3D=5m, 1W=1h, 1M=4h, 6M=1d, 1Y=1d, 3Y/5Y=1w, ALL=1M. Results are cached in IndexedDB (`tayrax-history`, DB version 2) with TTLs tied to interval duration (5m→5min cache, 1h→1h, 4h→4h, 1d/1w→24h, 1M→48h). The ALL preset shows the complete monthly history available on Binance; for most assets this goes back to 2017–2018. When fewer candles than the configured limit are returned, `History.svelte` reduces `effectiveDisplayCount` to ensure at least 34 warmup candles (MACD minimum) remain hidden.
...
- **Help entries** — each term/description pair in `Help.svelte` is a native `<details>`/`<summary>` element, collapsed by default. No JS — the browser handles expand/collapse. A CSS `▶` arrow on `summary::before` rotates 90° on `details[open]`. Do not replace this with a JS accordion or custom toggle pattern.
- **Button group wrapping** — any flex container that holds a row of buttons (preset selectors, toggle groups, control bars) must have `flex-wrap: wrap` so buttons reflow onto multiple lines on narrow screens instead of overflowing off-screen. On portrait viewports (≤480 px), button group containers that share a row with other controls (e.g. a dropdown) should additionally take full width (`flex: 0 0 100%`) so the group gets its own row and wraps across the full available width. Companion action buttons (e.g. a refresh button that belongs to a preset row) should be placed inside the same wrapping container rather than as a sibling flex item, so they stay visually attached to the group on all screen sizes.
- **Chart settings panel** — the ⚙ button in the chart header toggles an inline `settings-panel` div between the header and the SVG. It is always the last item in the sub-pane controls group (alongside RSI/MACD), separated from them by a `.sep` spacer. On wide screens all controls appear in one row (intervals | RSI MACD | ⚙); on portrait/narrow screens (≤480 px) the header stacks: asset name, then interval buttons row, then sub-pane + ⚙ row. Settings are per-component-instance (not persisted) and affect only the chart's visual overlays — not alerts or proposals. On portrait screens (≤480 px) each settings fieldset (SMA, EMA, BB, RSI, MACD) takes full width (`flex: 0 0 100%`) and its labels are allowed to wrap (`flex-wrap: wrap`) so that groups with three inputs (e.g. MACD Fast/Slow/Signal) never overflow — the third input wraps to a second line rather than being clipped. Input boxes are narrowed from `3.5rem` to `2.5rem` at this breakpoint to fit two labels side-by-side on most narrow viewports.
- **History analysis panel** — the `analysis-panel` `<div>` sits immediately below `<Chart>` inside `History.svelte`'s `{:else}` block. It contains: (a) a header row with an "Analysis" label and a button group (`analysis-btns`), and (b) a results section that appears only after a run. The button group follows the same `flex-wrap: wrap` + portrait full-width rule as preset buttons. Active mode is indicated with the same `preset-btn.active` styling used by preset buttons. Confidence badges use inline `style` with literal hex colors (not CSS tokens) because they are data-driven. Clicking a button always clears the previous results before running — there is never a state where two modes' results are shown simultaneously. The analysis panel is only rendered when `candles.length > 0` (i.e. not during loading or error states).
