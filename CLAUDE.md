...
│   │   └── bot-client.ts     # BotClient interface + SharedWorkerBotClient + createBotClient()
│   ├── components/
│   │   ├── PriceCard.svelte
│   │   ├── AlertForm.svelte
│   │   ├── AlertList.svelte
...
- `vi.mock()` factories are hoisted before class declarations in the test file body. Any variable referenced inside a `vi.mock` factory must be created with `vi.hoisted()`.
...
- **SharedWorker bot** — `src/lib/bot.worker.ts` is a SharedWorker that owns both WebSocket feed instances (`BinancePriceFeed`/`BinanceKlineFeed`) and the `BotEngine` (alert evaluation, proposal evaluation, backfill, `logAction`). It stays alive as long as any tab from the origin is open, so feeds run even when navigating between pages. It broadcasts `priceTick`, `closedCandle`, `priceStatus`, `klineStatus`, `notify`, and `botState` messages to all connected tab ports. It boots by reading `enabledAssets` from localStorage. Both `App.svelte` and `Bot.svelte` connect via `createBotClient()` in `src/lib/bot-client.ts`, which returns a `SharedWorkerBotClient`. The `/bot/` page calls `createBotClient()` synchronously in the `<script>` block (not in `onMount`) so it is immediately available on first render and testable without microtask flushing; it sends `subscribeBotState` to receive `botState` broadcasts. **Dev note:** SharedWorker HMR does not work — after changing `bot.worker.ts`, terminate the worker via Chrome DevTools → Application → Shared workers, then reload.
