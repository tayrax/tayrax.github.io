...
import { candles, applyClosedCandle, prependCandles } from './candles';
...
describe('applyClosedCandle', () => {
  it('adds a candle to the store for the asset', () => {
    ...
    applyClosedCandle(makeClosedCandle(asset, 1000));
    const map = get(candles);
  ...
  it('appends subsequent candles in order', () => {
    const asset = uniqueAsset();
    applyClosedCandle(makeClosedCandle(asset, 1000, 100));
    applyClosedCandle(makeClosedCandle(asset, 2000, 200));
    const list = get(candles)[asset];
  ...
  it('deduplicates candles with the same openTime (new wins)', () => {
    const asset = uniqueAsset();
    applyClosedCandle(makeClosedCandle(asset, 1000, 100));
    applyClosedCandle(makeClosedCandle(asset, 1000, 200));
    const list = get(candles)[asset];
  ...
  it('trims to CANDLE_HISTORY_MAX entries', () => {
    ...
    for (let i = 0; i < CANDLE_HISTORY_MAX + 5; i++) {
      applyClosedCandle(makeClosedCandle(asset, i * 1000));
    }
    expect(get(candles)[asset]).toHaveLength(CANDLE_HISTORY_MAX);
  });
});
...
describe('prependCandles', () => {
  it('inserts historical candles before live candles', () => {
    const asset = uniqueAsset();
    applyClosedCandle(makeClosedCandle(asset, 5000));
    prependCandles(asset, [makeOHLCV(1000), makeOHLCV(2000), makeOHLCV(3000)]);
    const list = get(candles)[asset];
    expect(list[0].openTime).toBe(1000);
  ...
  it('does not overwrite live candles with historical ones (live wins)', () => {
    const asset = uniqueAsset();
    applyClosedCandle(makeClosedCandle(asset, 3000, 999));
    prependCandles(asset, [makeOHLCV(3000, 111)]);
    const list = get(candles)[asset];
    expect(list).toHaveLength(1);
  ...
  it('does nothing for an empty array', () => {
    const asset = uniqueAsset();
    applyClosedCandle(makeClosedCandle(asset, 1000));
    prependCandles(asset, []);
    expect(get(candles)[asset]).toHaveLength(1);
  });
  ...
  it('trims combined result to CANDLE_HISTORY_MAX', () => {
    ...
    applyClosedCandle(makeClosedCandle(asset, CANDLE_HISTORY_MAX * 1000));
