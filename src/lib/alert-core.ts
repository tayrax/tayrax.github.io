...
import { ALERT_COOLDOWN_MS, type CandleInterval } from './config';
...
// ---------------------------------------------------------------------------
// localStorage helpers — used by the worker to persist the authoritative
// alerts list. Not used by tab-side code (tab stores are mirrors).
// ---------------------------------------------------------------------------
import { STORAGE_KEYS } from './config';

type StoredPayload = { version: 1; alerts: StoredAlert[] };

export const loadAlertsFromStorage = (): StoredAlert[] => {
  try {
...
export const persistAlertsToStorage = (alerts: StoredAlert[]): void => {
...
export const makeAlertId = (): string =>
